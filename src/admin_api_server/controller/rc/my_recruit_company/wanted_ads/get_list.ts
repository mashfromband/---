...
type ResponseType =
    paths["/rc/my-recruit-company/wanted-ads"]["get"]["responses"]["200"]["content"]["application/json"];
type OneAdsType =
    components["schemas"]["getRecruitCompanyWantedAdsListResponse"];

export class GetMyRecruitCompanyWantedAdsListAPIController extends BaseAPIController {
    public async getList() {
        const [wantedAdsList, total] = await DBUtil.getEntityManager(async (manager: EntityManager) => {
            const query = manager
                ...
                .where("rcwa.recruteCompanyId = :recruitCompanyId")
                .setParameters({
