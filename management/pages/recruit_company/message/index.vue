    ...
    >
      ...
      <div class="flex flex-col col-span-full lg:col-span-4">
        ...
        <div v-if="roomItemList.length" class="flex flex-col gap-2">
          <NuxtLink v-for="message in roomItemList" :key="message.id" :to="'/recruit_company/message/' + message.id">
            <UCard
              :ui="{
...
const loadPage = async () => {
  ...
  const { data } = await useCallApi<RoomListResponse>(
    apiEndpoint.value.endpoint + "/v1/rc/user/me/message-room",
    {
      method: "get",
      query: { offset, limit },
    },
...
const onDeleteConfirm = async () => {
  ...
  const { error } = await useCallApi(
    apiEndpoint.value.endpoint + "/v1/rc/user/me/message-room/" + deletingRoomItem.value.id,
    { method: "delete" },
  );
  if (error.value) {
