...
export const useApiEndpoint = () => useState<{
    ...
    () => {
        ...
        return {
            endpoint: runtimeConfig.public.adminAuthApiEndpoint,
        };
    },
);
...
const getAccessTokenUseRefreshToken = async () => {
    const refreshTokenCookie = useCookie("RL_M_RFT");
    ...
    const runtimeConfig = useRuntimeConfig();
    const url = runtimeConfig.public.authApiEndpoint as string + "/v1/admin/auth/accessToken";
    ...
    try {
        ...
        if (import.meta.server) {
            setRequestHeaderCookie("RL_M_ACT", response.accessToken);
            setRequestHeaderCookie("RL_M_RFT", response.refreshToken);
        }
        return response;
    } catch (err) {
        console.error(err);
...
export const useCallApi = async <ResT>(
    ...
    const refresh = async () => {
        ...
        for (let retries = 0; retries < 3; retries++) {
            const accessToken = useCookie("RL_M_ACT").value;
                    ...
                    if (!withoutAuthorization && responseStatus == 401) {
                        if (!options.unuseReloadAccessToken) {
                            if (await getAccessTokenUseRefreshToken()) {
                                ...
                                const newAccessToken = useCookie("RL_M_ACT").value;
