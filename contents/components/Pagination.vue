<template>
  <div class="flex items-center justify-between border-gray-200 py-6">
        ...
        <div class="flex flex-wrap items-center text-sm">
          <div>
            <template v-if="$props.total">
              <span class="font-medium">{{ firstNumber }}</span>～</template>
              <span class="font-medium">{{ lastNumber }}</span>件を表示中
          </div>
          <div>
            （全<span class="font-medium text-xl text-primary">{{ $props.total }}</span>件）
          </div>
        </div>
      </div>
      <UButton
