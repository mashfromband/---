<template>
  <div class="bg-el_yellow-50 dark:bg-gray-800/50 py-4 px-0 lg:p-8 lg:rounded-t-3xl">
    ...
    >
      <ULink :to="item.to">
        <UCard class="mx-4 my-1 w-96 relative "
          :ui="{
    ...
    <div
      ...
      class="lg:hidden h-[200px] flex items-center justify-center text-center"
    >
    ...
    <div
      v-else-if="feed.list && feed.list.length === 0"
      class="hidden h-[200px] lg:flex items-center justify-center text-center"
    >
      {{`${props.feed.title}がありません` }}
    </div>
    <div
      v-else
      class="hidden h-[200px] lg:flex items-center justify-center text-center animate-pulse"
    >
      読み込み中...
    </div>
  </div>
